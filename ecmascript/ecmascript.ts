...
const turmaB: string[] = ['Fernando', 'Lorena', ...turmaA]
console.log(turmaB)
...
async function examplePromiseAsyncAwait() {
    try {
        let resultado = await fetch('https://swapi.dev/api/people/1/');
        let res = await resultado.json()
        res = await res.films
        res = await fetch(res[0])
        res = await res.json()
        console.log(res)
    } catch (e) {
        console.log(e)
